...
class cms extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['pageTitle', 'slug', 'pageContent', 'author'], 'required'],
            [['pageContent', 'pageMetaDescription', 'pageStatus'], 'string'],
             ['pageTitle', 'unique',  'message' => 'Page title has already been taken.'],
            [['pageMetaTitle'], 'string', 'max' => 100],
            [['pageTitle', 'pageMetaKewords'], 'string', 'max' => 150],
            [['slug'], 'string', 'max' => 255],
            [['pageTitle', 'slug', 'pageContent', 'pageMetaTitle', 'pageMetaKewords', 'pageMetaDescription', 'author','pageStatus', 'modifiedDate'], 'safe'],
        ];
    }
        ...
        public function actionTaken($argArrData){
            ...
            if($argArrData){
                $varEcho = ($argArrData['status']=='1') ? 'Active' : ($argArrData['status']=='0') ? "Inactive" : "Deleted";
                 Cms::findOne($argArrData['data'])->delete();
                 echo $varEcho;
            }
        }
   ...
   /* public function search($params='')
    {
      mail("sathish.kumar1@mail.vinove.com","getvalue",print_r($_GET,1));
        $query = cms::find();
    ...
    mail("sathish.kumar1@mail.vinove.com","tesetttt",$query);
